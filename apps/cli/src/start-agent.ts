...
type InjectSpec = {
  ...
  /** Auto-accept / yolo mode config (project-level settings file path + content generator) */
  autoAcceptConfig?: {
    /** Relative path within project (e.g. ".claude/settings.json") or null for global-only */
    projectPath: string | null;
    /** Global path (via HOME env var), or null if no global config supported */
    globalPath: string | null;
    /** Generator function that returns file content */
    generator: () => string;
    /** Whether the config is JSON or TOML */
    isToml: boolean;
...
/**
 * Merge auto-accept JSON config into existing config (shallow merge at top level).
 */
const mergeJsonAutoAccept = (existingContent: string, newContent: string): string => {
  let existing: Record<string, unknown>;
  let next: Record<string, unknown>;
  try { existing = JSON.parse(existingContent); } catch { existing = {}; }
  try { next = JSON.parse(newContent); } catch { next = {}; }
  const merged = { ...existing, ...next };
  return JSON.stringify(merged, null, 2) + "\n";
};

/**
 * Merge auto-accept TOML config into existing config (append if not present).
 */
const mergeTomlAutoAccept = (existingContent: string, newContent: string): string => {
  ...
  switch (ide) {
    ...
    case "cursor":
      return {
        ...
        autoAcceptConfig: {
          ...
          merge: true,
          generator: () => JSON.stringify({
            "cursor.aiCodegen.enableAutoApply": true
          }, null, 2) + "\n"
        }
      };
    ...
    case "aider":
      return {
        ...
        autoAcceptConfig: {
          projectPath: ".aider.conf.yml",
          globalPath: null,
          isToml: false,
          merge: false,
          generator: () => `yes-always: true\n`
        }
      };
...
// ---------------------------------------------------------------------------
// setupAutoAccept: standalone command to configure auto-accept / YOLO mode
// for one or more AI coding tools. Not part of start-agent — user must run
// this explicitly.
// ---------------------------------------------------------------------------

export type SetupAutoAcceptOptions = {
  ide: SupportedIde;
  projectRoot: string;
  dryRun?: boolean;
};

export type SetupAutoAcceptResult = {
  success: boolean;
  path: string | null;
  message?: string;
};

export const setupAutoAccept = async (
  options: SetupAutoAcceptOptions
): Promise<SetupAutoAcceptResult> => {
  // auto-accept 配置不依赖 role，用 "host" 作为占位
  const injectSpec = getInjectSpec(options.ide, "host");

  if (!injectSpec.autoAcceptConfig) {
    return {
      success: false,
      path: null,
      message: `${options.ide} 不支持自动配置 auto-accept，需在 IDE 设置面板手动开启`,
    };
  }

  const ac = injectSpec.autoAcceptConfig;

  if (options.dryRun) {
    const targetPath = ac.projectPath
      ? join(options.projectRoot, ac.projectPath)
      : ac.globalPath
        ? join(getUserHome(), ac.globalPath)
        : null;
    return {
      success: true,
      path: targetPath,
      message: `[dry-run] 将写入 auto-accept 配置到: ${targetPath}`,
    };
  }

  const autoAcceptContent = ac.generator();

  if (ac.projectPath) {
    // 项目级配置
    const autoAcceptPath = join(options.projectRoot, ac.projectPath);
    await mkdir(dirname(autoAcceptPath), { recursive: true });

    if (ac.merge) {
      let existing = "";
      try { existing = readFileSync(autoAcceptPath, "utf-8"); } catch { /* new file */ }
      const merged = ac.isToml
        ? mergeTomlAutoAccept(existing, autoAcceptContent)
        : mergeJsonAutoAccept(existing, autoAcceptContent);
      await writeFile(autoAcceptPath, merged, "utf-8");
    } else {
      let existing = "";
      try { existing = readFileSync(autoAcceptPath, "utf-8"); } catch { /* new file */ }
      const combined = existing.trimEnd() + (existing.trimEnd() ? "\n" : "") + autoAcceptContent;
      await writeFile(autoAcceptPath, combined, "utf-8");
    }

    return { success: true, path: ac.projectPath };
  } else if (ac.globalPath) {
    // 全局级配置
    const home = getUserHome();
    const globalPath = join(home, ac.globalPath);
    await mkdir(dirname(globalPath), { recursive: true });
    await writeFile(globalPath, autoAcceptContent, "utf-8");
    return { success: true, path: ac.globalPath };
  }

  return { success: false, path: null, message: "无可用配置路径" };
};