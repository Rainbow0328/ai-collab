...
- `status` — 查看当前会话快照状态
- `start_worker` — 启动新的 AI Worker 并自动加入当前会话（Host 专属）
...
Host 可以使用 `start_worker` 工具按需启动新的 AI Worker，自动加入当前会话。

使用场景：
- 当前没有足够的 Worker 处理任务。
- 某类任务需要特定 AI 工具的能力（如 Claude 擅长推理，Cursor 擅长代码编辑）。
- 用户要求增加并行度。

调用方式：
- `start_worker(ide="claude", duty="前端开发")` — 启动一个 Claude Code worker，职责为前端开发。
- `start_worker(ide="cursor", duty="UI 组件", name="ui-worker")` — 启动一个 Cursor worker，自定义名称。

支持的 IDE：claude, codex, cursor, trae, opencode, gemini, aider, windsurf, qoder, github, cline, crusher, lov, mimo。

行为规则：
1. 启动 Worker 前必须先通过 `attach` 加入会话。
2. Worker 会自动继承当前会话名称和工作目录。
3. Worker 启动后，Host 不需要等待 Worker `attach`，可以直接 `dispatch_many` 派发任务。
4. 如果 Worker IDE 未安装或启动失败，`start_worker` 会返回错误信息，Host 应向用户说明并建议安装。
5. 不要无限制启动 Worker，根据任务需要合理控制并行度。
