...
1. 确认当前没有未完成的任务（所有 Worker 已回报或没有 `PROCESS_CLAIMED_MESSAGE`）。
2. 确认知识库 L1 已记录当前会话方向和关键决策。
3. 建议用户清空上下文。
4. 清空后，调用 `resume` 工具一键恢复：
   - `resume` 自动执行 `attach`（复用已有绑定）+ 读取 L1 方向 + 列出成员。
5. 根据 `resume` 返回的 L1 方向和成员状态，决定下一步操作。
...
- 完成一个完整协作阶段（如一个功能模块完成）后，建议 clear + resume。
- 派发新一批任务前，如果上下文已接近上限，建议 compact 或 clear。
- 使用 `status` 工具检查会话状态，判断是否是 compact/clear 的好时机。
...
| 一个完整功能模块已交付，开始新模块 | clear + resume |
| 不确定 | 先 compact，如不够再 clear |

### clear 安全条件检查

在建议 clear 之前，Host 必须确认以下条件全部满足：

1. 没有待处理的 `PROCESS_CLAIMED_MESSAGE`。
2. 所有已派发任务都已收到 Worker 回报。
3. L1 知识库已记录当前会话方向和关键约束。
4. 如果有未完成的知识库裁决，已通过 `knowledge_upsert` 写入。

如果以上任一条件不满足，必须先完成相应操作再 clear，或改用 compact。
