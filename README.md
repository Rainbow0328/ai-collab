...
| **Auto-Accept** | 一键为 AI 工具配置 YOLO 模式（`auto-accept claude codex` 或 `auto-accept all`） |
...
### Auto-Accept / YOLO 模式配置

为 AI 编程工具自动配置"自动批准文件修改"模式（合并写入，不覆盖已有配置）：

```bash
# 为单个工具配置
loopmarshal auto-accept claude
loopmarshal auto-accept codex

# 为多个工具同时配置
loopmarshal auto-accept claude codex cursor aider

# 为所有支持的工具配置
loopmarshal auto-accept all

# 预览不写入
loopmarshal auto-accept all --dry-run
```
...
| AI 工具 | 配置文件 | 配置内容 |
|---------|---------|---------|
| Claude Code | `.claude/settings.json` | `permissions.allow: ["Bash(*)","Read(*)","Write(*)","Edit(*)"]` |
| Codex CLI | `.codex/config.toml` | `[approval] full_auto = true` |
| Cursor | `.cursor/settings.json` | `cursor.aiCodegen.enableAutoApply: true` |
| Aider | `.aider.conf.yml` | `yes-always: true` |

> ⚠️ Auto-accept / YOLO 模式会自动批准 AI 的文件修改请求，请在受信任的项目中使用。
